...
import { getNewsPosts } from '~/integrations/strapi/data-fetcher';
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  const blogPosts = await getNewsPosts(searchParams);
...
export default async function BlogPostPage({ params: { locale }, searchParams }: Props) {
  const blogPosts = await getNewsPosts({ locale, ...searchParams });
  ...
  return (
    <div className="mx-auto max-w-screen-xl">
      ...
      <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3 lg:gap-8">
        {blogPosts.posts.items.map((post: any) => {
